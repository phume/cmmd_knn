"""
Phase 4 Runner: Real/Semi-Synthetic Datasets

Runs experiments on:
- UCI Adult (original + synthetic injection)
- UCI Bank Marketing (original + synthetic injection)
- Cardio (ODDS)
"""
...
LOGS_DIR = ROOT / 'logs'
RAW_CSV = RESULTS_DIR / 'phase4_raw.csv'
SUMMARY_CSV = RESULTS_DIR / 'phase4_summary.csv'
ERRORS_CSV = RESULTS_DIR / 'phase4_errors.csv'
STATE_FILE = RESULTS_DIR / 'phase4_state.json'
...
def get_all_methods(seed, n_samples):
    ...
    K = min(100, n_samples // 20)  # Adaptive K based on dataset size
    return {
        ...
        'PNKDIF': PNKDIF(PNKDIFConfig(n_neighbors=K, random_state=seed)),
        'PNKDIF_uniform': PNKDIFUniform(PNKDIFConfig(n_neighbors=K, random_state=seed)),
        'PNKDIF_noMLP': PNKDIFNoMLP(PNKDIFConfig(n_neighbors=K, random_state=seed)),
    }
...
def run_phase4():
    logger.info("=" * 60)
    logger.info("PHASE 4: Real/Semi-Synthetic Datasets")
    logger.info("=" * 60)
    ...
    total_runs = len(DATASETS) * len(SEEDS) * 10  # 10 methods
    run_idx = 0
        ...
        for seed in SEEDS:
            try:
                ...
                for method_name, model in methods.items():
                    ...
                    logger.info(f"  [{run_idx}] {method_name} | seed={seed}")
