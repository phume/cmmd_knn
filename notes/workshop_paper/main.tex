...
PNKIF is \emph{not} proposed as a novel anomaly detection paradigm. Instead, it serves as a minimal, interpretable reference implementation designed to isolate the effect of peer-based normalization on anomaly scoring.

By combining kernel-weighted peer normalization with a standard Isolation Forest scorer, PNKIF deliberately avoids learned representations, end-to-end optimization, or deep architectures. This design choice ensures that observed performance differences can be attributed to \emph{contextual normalization}, rather than representational capacity or training artifacts.

The method applies peer normalization before Isolation Forest:

\begin{enumerate}
    \item \textbf{Compute peer weights:} For each point $i$, compute RBF kernel weights based on context similarity:
    \[
    w_{ij} = \exp\left(-\frac{\|\mathbf{c}_i - \mathbf{c}_j\|^2}{2\gamma^2}\right)
    \]

    \item \textbf{Compute peer statistics:} Weighted mean and standard deviation:
    \[
...
A key finding of this study is that \emph{agreement and disagreement} between global and contextual detectors is itself informative.

Empirically, we observe three regimes:

\begin{itemize}
    \item \textbf{Agreement at high scores:} Anomalies are globally unusual; context adds little value.
    \item \textbf{Contextual-only detection:} Behavior is common globally but anomalous for its peer group.
    \item \textbf{Widespread disagreement:} The dataset contains substantial contextual structure.
\end{itemize}

This suggests a practical diagnostic workflow: practitioners can run a global detector (e.g., IF) alongside a contextual detector (e.g., PNKIF). Divergence between the two signals that context-aware modeling is warranted, while agreement indicates that global detection is sufficient.
...
More importantly, we propose reframing contextual detection as a \emph{diagnostic tool}: disagreement between global and contextual methods signals the presence of contextual structure. This provides practitioners with actionable guidance beyond method selection.
