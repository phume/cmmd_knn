...
\subsection{Positioning of PNKDIF}

Table~\ref{tab:comparison} positions PNKDIF relative to existing methods. Our method uniquely combines: (1) context-awareness via peer normalization, (2) non-linear decision boundaries via random projections, and (3) training-free operation.
...
\midrule
IF \cite{liu2008isolation} & \texttimes & \texttimes & \checkmark \\
DIF \cite{xu2023deep} & \texttimes & \checkmark & \checkmark \\
QCAD \cite{liang2021conditional} & \checkmark & \texttimes & \checkmark \\
ROCOD \cite{liang2023rocod} & \checkmark & \texttimes & \checkmark \\
CVAE \cite{sohn2015learning} & \checkmark & \checkmark & \texttimes \\
CWAE & \checkmark & \checkmark & \texttimes \\
\textbf{PNKDIF (Ours)} & \checkmark & \checkmark & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
